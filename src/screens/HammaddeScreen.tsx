...
export const HammaddeScreen: React.FC = () => {
  ...
  const [editMinStock, setEditMinStock] = useState<number>(100);
  ...
  // Mal Kabul / Yeni Parti Formu (FR-11)
  const [batchNo, setBatchNo] = useState('');
  const [initialQty, setInitialQty] = useState<number>(500);
  const [expiryDate, setExpiryDate] = useState('');
  const [batchLocationId, setBatchLocationId] = useState('');
  const [labelQty, setLabelQty] = useState<number>(50);
  ...
  const fetchData = async () => {
    ...
    try {
      const [prodRes, supRes, locRes] = await Promise.all([
        api.get('/api/products?category=HAMMADDE'),
      ...
      console.error('Hammadde verileri alınamadı:', err);
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleCreateProduct = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const hammaddeCat = catRes.data.find((c: any) => c.code === 'HAMMADDE');

      await api.post('/api/products', {
        code: newCode.trim().toUpperCase(),
        name: newName.trim(),
        categoryId: hammaddeCat.id,
        unit: newUnit,
      ...
      setNewName('');
      fetchData();
      setSuccessMsg('Yeni hammadde kartı başarıyla açıldı.');
  ...
  const handleCreateBatch = async (e: React.FormEvent) => {
    ...
    try {
      ...
      fetchData();
      setSuccessMsg(
        `Mal kabul başarılı: ${selectedProduct.name} için ${initialQty} ${selectedProduct.unit} stoğa eklendi. ${res.data.labels?.length || 0} adet QR etiket oluşturuldu.`,
      );
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-white p-6 rounded-2xl border border-slate-200 shadow-sm">
        <div>
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-xl font-bold text-slate-800">Hammadde Stok Yönetimi</h2>
          </div>
          <p className="text-xs text-slate-500 mt-1">
            Çiğ süt, peynir mayası, tuz vb. üretim hammaddelerinin parti ve SKT bazlı takibi (FR-01, FR-04).
          </p>
        </div>

        <button
          onClick={() => setShowProductModal(true)}
          className="flex items-center gap-2 px-4 py-2.5 bg-amber-500 hover:bg-amber-600 text-slate-950 font-bold rounded-xl text-sm shadow-md shadow-amber-500/20 transition shrink-0"
        >
          <Plus className="w-4 h-4" /> Yeni Hammadde Kartı Aç
        </button>
      </div>
      ...
      <div className="bg-white p-4 rounded-xl border border-slate-200 flex items-center gap-3">
        ...
        <input
          ...
          placeholder="Hammadde adı veya ürün koduna göre filtrele..."
                  ...
                  <td className="py-3 px-4 text-right flex items-center justify-end gap-1.5">
                    ...
                    <button
                      onClick={() => {
                        ...
                        setBatchNo(`PRT-${new Date().toISOString().slice(2, 10).replace(/-/g, '')}`);
                      }}
                    ...
                    >
                      <Plus className="w-3.5 h-3.5" /> Mal Kabul
                    </button>
                  </td>
                </tr>
              ))}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm animate-in fade-in">
          <div className="bg-white rounded-2xl max-w-md w-full p-6 shadow-2xl border border-slate-200 space-y-4">
            <div className="flex items-center justify-between border-b pb-3">
              <h3 className="font-bold text-slate-800 text-lg">Yeni Hammadde Kartı</h3>
            ...
            <form onSubmit={handleCreateProduct} className="space-y-3.5">
              ...
              <div>
                <label className="block text-xs font-bold text-slate-700 mb-1">Hammadde Adı *</label>
                <input
                  type="text"
                  required
                  placeholder="Örn: Yoğurt Kültürü"
                  value={newName}
              ...
              <div>
                ...
                <input
                  type="text"
                  placeholder="Kullanım yeri veya saklama koşulu"
              ...
              <div className="pt-2 flex justify-end gap-2">
                ...
                <button
                  ...
                  className="px-5 py-2 bg-amber-500 hover:bg-amber-600 text-slate-950 font-bold rounded-xl text-xs shadow transition"
                >
                  Kaydet
                </button>
              </div>
      ...
      {/* Mal Kabul & Yeni Parti Modalı (FR-11) */}
      {showBatchModal && selectedProduct && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm animate-in fade-in">
          <div className="bg-white rounded-2xl max-w-lg w-full p-6 shadow-2xl border border-slate-200 space-y-4">
            <div className="flex items-center justify-between border-b pb-3">
              <div>
                <h3 className="font-bold text-slate-800 text-lg">Depo Mal Kabul & Etiket Üretimi (FR-11)</h3>
            ...
            <form onSubmit={handleCreateBatch} className="space-y-3.5">
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-xs font-bold text-slate-700 mb-1">Parti / Lot Numarası *</label>
                  <input
                    type="text"
                    required
                    value={batchNo}
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-xs font-bold text-slate-700 mb-1">Son Kullanma Tarihi (SKT)</label>
                  <input
                    type="date"
                    value={expiryDate}
              ...
              <div className="p-3 bg-blue-50/60 rounded-xl border border-blue-200">
                <label className="block text-xs font-bold text-blue-900 mb-1">
                  Her Etikete Düşen Paket/Koli Miktarı ({selectedProduct.unit})
                </label>
                <input
                  type="number"
                  min="0.01"
                ...
                <span className="text-[11px] text-blue-700 mt-1 block">
                  Sistem otomatik olarak {labelQty > 0 ? Math.ceil(initialQty / labelQty) : 1} adet benzersiz QR kodlu etiket üretecektir.
                </span>
              </div>
      ...
      {/* Ürün / Raf QR Kod Modalı */}
      {showProductQrModal && productQrData && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm animate-in fade-in">
          <div className="bg-white rounded-2xl max-w-sm w-full p-6 shadow-2xl border border-slate-200 space-y-4">
            <div className="flex items-center justify-between border-b pb-3">
              <h3 className="font-bold text-slate-800 text-sm">Ürün / Raf Sabit QR Kodu</h3>
      ...
      {showEditModal && selectedProduct && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm animate-in fade-in">
          <div className="bg-white rounded-2xl max-w-md w-full p-6 shadow-2xl border border-slate-200 space-y-4">
            <div className="flex items-center justify-between border-b pb-3">
              <div>
                <h3 className="font-bold text-slate-800 text-lg">Hammadde Kartını Düzenle</h3>
      ...
      {showDetailModal && productDetail && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm animate-in fade-in">
          <div className="bg-white rounded-2xl max-w-lg w-full p-6 shadow-2xl border border-slate-200 space-y-4">
            ...
            {/* Aktif Partiler / Lotlar (FR-04) */}
            <div className="space-y-2">
              <h4 className="text-xs font-bold text-slate-700 uppercase tracking-wider">Kayıtlı Partiler (Lot)</h4>
              <div className="divide-y divide-slate-100 border rounded-xl overflow-hidden max-h-40 overflow-y-auto">
                {productDetail.batches?.length === 0 ? (
                  <div className="p-3 text-xs text-slate-400 text-center">Bu ürüne ait parti bulunmuyor.</div>
                ) : (
                    ...
                    <div key={b.id} className="p-2.5 flex items-center justify-between text-xs hover:bg-slate-50">
                      <div>
                        ...
                        <span className="text-[10px] text-slate-400">
                          SKT: {b.expiryDate ? new Date(b.expiryDate).toLocaleDateString('tr-TR') : 'YOK'}
                        </span>
                      </div>
